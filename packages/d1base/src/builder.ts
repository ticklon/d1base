...
export class QueryBuilder<T extends Record<string, any> = Record<string, any>> {
   ...
   * リレーションを定義する
   * @param name リレーション名
   * @param config リレーション設定
   */
  defineRelation(name: string, config: RelationConfig): QueryBuilder<T> {
    if (!this.state.relations) {
      this.state.relations = {};
    }
    this.state.relations[name] = config;
    return this;
  }
   ...
   */
  select(columns: (keyof T) | (keyof T)[] | Record<string, any> = '*'): QueryBuilder<T> {
    if (typeof columns === 'string') {
      if (columns === '*') {
        this.state.select = [];
      } else {
        this.state.select = [columns as string];
      }
    } else {
      this.state.select = columns as any;
    }
    return this;
  }
   ...
   */
  order(column: keyof T | string, direction: 'asc' | 'desc' = 'asc'): QueryBuilder<T> {
    if (!this.state.order) {
      this.state.order = [];
    }
